...
<template>
  <div class="w-full select-none">
    ...
    <div class="relative h-[80px] w-full">
      ...
      <Transition name="fade">
        <div v-if="!isReady" class="absolute inset-0">
          <div class="flex items-end gap-0.5 h-full w-full justify-center">
            <div v-for="(h, i) in FAKE_BARS" :key="i" class="w-0.5 rounded-full animate-pulse" :style="{
              height: `${h}px`,
          ...
          <div class="absolute bottom-0 left-0 right-0 h-px bg-white/10 overflow-hidden">
            <div class="absolute inset-y-0 left-0 bg-white/50 transition-[width] duration-300"
      ...
      <div ref="containerRef" class="absolute inset-0 transition-opacity duration-500"
    ...
    <div class="flex flex-wrap items-center gap-x-3 gap-y-3 pt-3">
      ...
      <div class="order-1 sm:order-2 w-full sm:w-auto sm:flex-1 flex items-center justify-center gap-3">
        ...
        <button
          ...
          title="Retroceder 5s" @click="skip(-5)">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <path
              d="M11.99 5V1l-5 5 5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6h-2c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z" />
            <text x="12.5" y="15.5" text-anchor="middle" font-size="5.5" font-family="monospace" font-weight="bold"
        ...
        <button
          ...
          :title="isPlaying ? 'Pausar' : 'Reproducir'" @click="togglePlay">
          <svg v-if="!isPlaying" width="44" height="44" viewBox="0 0 24 24" fill="currentColor">
            <path d="M8 5v14l11-7z" />
          </svg>
          <svg v-else width="44" height="44" viewBox="0 0 24 24" fill="currentColor">
        ...
        <button
          class="w-9 h-9 sm:w-8 sm:h-8 flex items-center justify-center text-white/45 hover:text-white transition-colors duration-200"
          title="Adelantar 5s" @click="skip(5)">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <path
              d="M12.01 5V1l5 5-5 5V7c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6h2c0 4.42-3.58 8-8 8s-8-3.58-8-8 3.58-8 8-8z" />
            <text x="11.5" y="15.5" text-anchor="middle" font-size="5.5" font-family="monospace" font-weight="bold"
              fill="currentColor">5</text>
          </svg>
        </button>
      </div>
      ...
      <div class="order-2 sm:order-3 ml-auto sm:ml-0 flex items-center gap-2 sm:gap-3 shrink-0">

        <!-- WF / SP toggle -->
        <div class="flex items-center gap-1 sm:gap-1.5 shrink-0">
          <span class="text-[8px] sm:text-[9px] font-mono tracking-widest transition-colors duration-200"
            :class="!showSpectrogram ? 'text-white/70' : 'text-white/25'">WF</span>
          <button
            class="relative w-8 sm:w-9 h-[16px] sm:h-[18px] border border-white/20 transition-all duration-300 shrink-0"
            :class="showSpectrogram ? 'bg-white/10' : 'bg-transparent'" title="Forma de onda / Espectrograma"
          ...
          <span class="text-[8px] sm:text-[9px] font-mono tracking-widest transition-colors duration-200"
            :class="showSpectrogram ? 'text-white/70' : 'text-white/25'">SP</span>
        </div>
        ...
        <div class="flex items-center gap-0.5">
          <button
            ...
            title="Alejar" @click="zoomOut">−</button>
          <button
            class="w-6 h-6 flex items-center justify-center text-sm font-mono text-white/35 hover:text-white/75 transition-colors duration-200"
            title="Acercar" @click="zoomIn">+</button>
        </div>

        <!-- Mute -->
        <button
          class="w-6 h-6 flex items-center justify-center text-white/45 hover:text-white transition-colors duration-200 shrink-0"
          :title="isMuted ? 'Activar sonido' : 'Silenciar'" @click="toggleMute">
          <svg v-if="!isMuted && volume > 0.5" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
            <path
        ...
        <input type="range" min="0" max="1" step="0.02" :value="isMuted ? 0 : volume"
          class="volume-range w-16 sm:w-24 shrink-0" @input="onVolumeChange" />
      </div>
    </div>
